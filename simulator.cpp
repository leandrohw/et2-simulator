      ...
      case 'A':
        {
          ...
          if (debug > 0) {
            cout << "Allocate 0x" << hex << object_id << " size 0x" << size
                 << dec << " at time " << time << endl;
      ...
      case 'U':
        ...
        if (object_id != 0 && new_target != 0) {
          ...
          if (targetObject->getType() != "[C" &&
              ...
              troot->getId() != 0x19a) {
            if (debug > 0) {
              cout << "Pointer from 0x" << hex << object_id << " "
      ...
      case 'M':
        {
          ...
          if (curContext == 0) {
            ...
            if (curContext) {
              cout << "Spawn thread 0x" << hex << thread_id << dec
                   << " -- started at " << curContext->getMethodFullName()
                   << endl;
              cout << "   in context" << endl;
              curContext->printStack();
            } else {
              cout << "Problem: no threadStart for thread id 0x" << hex
                   << thread_id << dec << endl;
              curContext = root;
            }
          }
          ...
          if (debug > 0 || new_thread) {
            cout << "Enter " << curContext->getMethodFullName() << " 0x"
                 << hex << method_id << " thread 0x" << thread_id << dec
                 << " at time " << time << endl;
            if (debug > 1) curContext->printStack();
          }
      ...
      case 'E':
        {
          ...
          if (curContext == 0) {
            ...
            cout << "ERROR: Thread 0x" << hex << thread_id
                 << " returning from method " << meth->getMethodName()
                 << " in class " << meth->getClassName() << endl;
            break;
          }

          if (debug > 0) {
            cout << "Exit  " << curContext->getMethodFullName() << " 0x"
                 << hex << method_id << " thread 0x" << thread_id << dec
                 << " at time " << time << endl;
            if (debug > 1) curContext->printStack();
          }
